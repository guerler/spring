...
struct HHsearch {
    ...
    bool construct(string chain, string modelpath, string fname, double minscore = -LARGE, double seqcut = LARGE) {
        ...
        while (f.good()) {
            ...
            if (f.get(0) == "Query") {
                query = f.get(1);
                int querypos = query.find(':');
                if (querypos != -1) {
                    query = query.substr(0, querypos);
                }
            }
    ...
    bool getmodel (SpecMolecule* mol, string templatename, SpecMolecule* moltmpl) {
        ...
        while (f.good()) {
            ...
            if (step == 2 && k == 'T' && f.get(1) == templatename) {
                ...
                // collect alignment
                querycomplete += queryseq;
                tmplcomplete += tmplseq;
            }
        }
        ...
        // log
        Msg::write ("Query    : %s", querycomplete.c_str());
        Msg::write ("Template : %s", tmplcomplete.c_str());
